...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS properties (
    ...
    owner_id TEXT, type TEXT, prop_id TEXT, name TEXT, price INTEGER, income INTEGER,
  ...
  CREATE TABLE IF NOT EXISTS mine_inventory (
    id INTEGER PRIMARY KEY AUTOINCREMENT, owner_id TEXT, resource_name TEXT,
    quantity INTEGER DEFAULT 0, unit_value INTEGER DEFAULT 0
  );
...
// ✅ مناجم محدثة (12 منجم) مع إصلاح أسماء الموارد لكلمة واحدة
const MINES_LIST = [
...
const TRANSFER_COOLDOWN = 1 * 60 * 60 * 1000; // ✅ ساعة واحدة
...
async function updateWealthRole(member, balance) {
  if(!member) return;
  try {
    if(CONFIG.MILLIONAIRE_ROLE_ID&&balance>=10_000_000){const role=member.guild.roles.cache.get(CONFIG.MILLIONAIRE_ROLE_ID);if(role&&!member.roles.cache.has(CONFIG.MILLIONAIRE_ROLE_ID))await member.roles.add(role);}
    else if(CONFIG.RICH_ROLE_ID&&balance>=1_000_000){const role=member.guild.roles.cache.get(CONFIG.RICH_ROLE_ID);if(role&&!member.roles.cache.has(CONFIG.RICH_ROLE_ID))await member.roles.add(role);}
  } catch(e){console.error('خطأ رول:',e);}
}

async function sendLog(client, guildId, content) {
  if(!CONFIG.LOG_CHANNEL_ID) return;
  try{const guild=client.guilds.cache.get(guildId);const channel=guild?.channels.cache.get(CONFIG.LOG_CHANNEL_ID);if(channel)await channel.send(content);}catch(e){}
}
...
client.on('messageCreate', async (msg) => {
  ...
  try {
    ...
    if (cmd==='ملفي'||cmd==='بروفايل') {
      ...
      const level=getLevel(u.balance+u.bank), props=db.prepare('SELECT * FROM properties WHERE owner_id=?').all(u.id);
      const income=props.reduce((s,p)=>s+p.income,0), prot=u.protection_until>Date.now();
      const myMine=db.prepare('SELECT * FROM user_mines WHERE owner_id=?').get(u.id), repLvl=getReputation(u.reputation);
      return msg.reply({embeds:[new EmbedBuilder()
        ...
        .addFields(
          ...
          {name:'🏆 المستوى',value:level.name,inline:true},{name:'⭐ السمعة',value:`${u.reputation}/100 ${repLvl.name}`,inline:true},{name:'📈 دخل/ساعة',value:fmt(income),inline:true},
          {name:'🏠 الممتلكات',value:`${props.length}`,inline:true},{name:'⛏️ المنجم',value:myMine?MINES_LIST.find(m=>m.id===myMine.mine_id)?.name:'لا يوجد',inline:true},
          {name:'💳 القرض',value:u.loan>0?fmt(u.loan):'لا يوجد',inline:true},{name:'🛡 الحماية',value:prot?`⏰ ${fmtTime(u.protection_until-Date.now())}`:'❌',inline:true},
    ...
    // ✅ إصلاح إيقاف الخدمات — راتب وعمل مسموح دائماً لرفع السمعة
    const repLvl=getReputation(user.reputation);
    ...
    if (cmd==='راتب') {
      ...
      const level=getLevel(user.balance+user.bank), tax=Math.floor(level.salary*0.01), net=level.salary-tax;
    ...
    if (cmd==='عمل') {
      ...
      const level=getLevel(user.balance+user.bank), earned=Math.floor(level.salary*0.3+Math.random()*level.salary*0.4);
      ...
      return msg.reply({embeds:[new EmbedBuilder().setTitle(jobs[Math.floor(Math.random()*jobs.length)]).setColor('#ffd700')
        .setDescription(`عملت وكسبت **${fmt(earned)}**!`).setFooter({text:'⭐ +1 سمعة | ارجع كل ساعة'})]});
    }
    ...
    if (cmd==='تحويل') {
      ...
      if(amount>maxTransfer) return msg.reply(`❌ الحد الأقصى للتحويل: **${fmt(maxTransfer)}** (50% من ثروتك)`);
    ...
    if (cmd==='شراء') {
      ...
      if(!type||!itemId||!['بيت','شركة','شركه'].includes(type)) return msg.reply('❌ الاستخدام: `شراء بيت h1` أو `شراء شركة c1` أو `شراء منجم m1`');
      ...
      return msg.reply({embeds:[new EmbedBuilder().setTitle(`✅ تم الشراء — ${item.name}`).setColor('#00ff88')
        .addFields({name:'💰 السعر',value:fmt(item.price),inline:true},{name:'📈 دخل/ساعة',value:fmt(item.income),inline:true},{name:'📅 دخل/يوم',value:fmt(item.income*24),inline:true})
    ...
    if (cmd==='بيع') {
      ...
      if (args[0]==='مورد') {
        ...
        return msg.reply({embeds:[new EmbedBuilder().setTitle('💰 تم بيع المورد!').setColor('#00ff88')
      ...
      db.prepare('UPDATE users SET balance=balance+? WHERE id=?').run(sellPrice,msg.author.id);
      return msg.reply({embeds:[new EmbedBuilder().setTitle(`💸 تم البيع — ${prop.name}`).setColor('#ffd700')
    ...
    if (cmd==='ممتلكاتي') {
      ...
      return msg.reply({embeds:[new EmbedBuilder().setTitle(`🏛 ممتلكات ${msg.author.username}`).setColor('#9b59b6')
        .setDescription(desc).addFields({name:'📈 دخل/ساعة',value:fmt(totalIncome),inline:true},{name:'📅 دخل/يوم',value:fmt(totalIncome*24),inline:true})
        .setFooter({text:'بيع [رقم] لبيع عقار'})]});
    }

    if (cmd==='تحصيل') {
      const left=cooldownLeft(user,'تحصيل'); if(left>0) return msg.reply(`⏰ انتظر **${fmtTime(left)}**`);
      const props=db.prepare('SELECT * FROM properties WHERE owner_id=?').all(msg.author.id);
      if(props.length===0) return msg.reply('❌ ليس لديك ممتلكات!');
      const now=Date.now(); let total=0;
      for(const p of props){total+=Math.floor(p.income*Math.min(24,(now-p.bought_at*1000)/3600000));db.prepare('UPDATE properties SET bought_at=? WHERE id=?').run(Math.floor(now/1000),p.id);}
      db.prepare('UPDATE users SET balance=balance+?, last_collect=? WHERE id=?').run(total,now,msg.author.id);
      return msg.reply({embeds:[new EmbedBuilder().setTitle('💰 تم التحصيل!').setColor('#00ff88').setDescription(`جمعت **${fmt(total)}** من **${props.length}** عقارات!`).setFooter({text:'ارجع بعد ساعة'})]});
    }
    ...
    if (cmd==='مستودعي') {
      ...
      return msg.reply({embeds:[new EmbedBuilder().setTitle(`📦 مستودع ${msg.author.username}`).setColor('#8B4513')
        ...
        .addFields({name:'💰 إجمالي القيمة',value:fmt(totVal)}).setFooter({text:'بيع مورد [اسم] [كمية/كل]'})]});
    }
    ...
    if (cmd==='قرض') {
      ...
      return msg.reply({embeds:[new EmbedBuilder().setTitle('🏦 تم منح القرض').setColor('#ffd700')
        .addFields({name:'💰 استلمت',value:fmt(amount),inline:true},{name:'💸 للسداد +10%',value:fmt(repay),inline:true},{name:'⏰ المهلة',value:'7 أيام',inline:true})
    ...
    if (cmd==='حماية') {
      ...
      return msg.reply({embeds:[new EmbedBuilder().setTitle('🛡 تم تفعيل الحماية!').setColor('#00ff88')
        .addFields({name:'⏰ المدة',value:`${opt.hours} ساعة`,inline:true},{name:'💰 التكلفة',value:fmt(opt.price),inline:true},{name:'🕐 تنتهي',value:`<t:${Math.floor(until/1000)}:R>`,inline:true})]});
    }
    ...
    if (cmd==='مجازفة') {
      ...
      return msg.reply({embeds:[new EmbedBuilder().setTitle(win?'🎉 ربحت!':'😭 خسرت!').setColor(win?'#00ff88':'#e74c3c')
        .addFields({name:'🎲 النتيجة',value:win?'✅ فزت':'❌ خسرت',inline:true},{name:'💰 المبلغ',value:fmt(amount),inline:true},{name:'💳 الرصيد الجديد',value:fmt(user.balance+change),inline:true})]});
    }
    ...
    if (cmd==='استثمر') {
      ...
      return msg.reply({embeds:[new EmbedBuilder().setTitle(win?'📈 استثمار ناجح!':'📉 خسارة!').setColor(win?'#00ff88':'#e74c3c')
        .addFields({name:'💰 المستثمر',value:fmt(amount),inline:true},{name:win?'📈 الربح':'📉 الخسارة',value:fmt(Math.abs(change)),inline:true},{name:'💳 الرصيد الجديد',value:fmt(user.balance+change),inline:true})]});
    }
    ...
    if (cmd==='تداول') {
      ...
      return msg.reply({embeds:[new EmbedBuilder().setTitle(`📊 ${result}`).setColor(change>=0?'#00ff88':'#e74c3c')
        .addFields({name:'💰 المبلغ',value:fmt(amount),inline:true},{name:change>=0?'📈 ربح':'📉 خسارة',value:fmt(Math.abs(change)),inline:true},{name:'💳 الرصيد الجديد',value:fmt(user.balance+change),inline:true})]});
    }
    ...
    if (cmd==='سمعة') {
      ...
      return msg.reply({embeds:[new EmbedBuilder().setTitle('⭐ نظام السمعة').setColor('#ffd700')
        .addFields(
          ...
          {name:'📈 كيف ترفع سمعتك؟',value:['`راتب` — +1 سمعة (يشتغل حتى عند إيقاف الخدمات)','`عمل` — +1 سمعة (يشتغل حتى عند إيقاف الخدمات)','`سداد` — +5 سمعة','`شراء بيت/شركة/منجم` — +2 سمعة'].join('\n')},
    ...
    if (['إحصائياتي','احصائياتي','احصاياتي','إحصاياتي'].includes(cmd)) {
      const props=db.prepare('SELECT * FROM properties WHERE owner_id=?').all(msg.author.id);
      const level=getLevel(user.balance+user.bank), totInc=props.reduce((s,p)=>s+p.income,0);
      const myMine=db.prepare('SELECT * FROM user_mines WHERE owner_id=?').get(msg.author.id), repLvl=getReputation(user.reputation);
      return msg.reply({embeds:[new EmbedBuilder().setTitle(`📊 إحصائيات — ${msg.author.username}`).setColor('#9b59b6')
        .addFields(
          ...
          {name:'🏠 بيوت',value:`${props.filter(p=>p.type==='house').length}`,inline:true},{name:'🏢 شركات',value:`${props.filter(p=>p.type==='company').length}`,inline:true},{name:'📈 دخل/ساعة',value:fmt(totInc),inline:true},
    ...
    if (cmd==='اوامر'||cmd==='أوامر') {
      return msg.reply({embeds:[new EmbedBuilder().setTitle('📋 قائمة الأوامر الكاملة').setColor('#4e8aff')
        .addFields(
          ...
          {name:'🏦 البنك',value:['`إيداع [مبلغ/كل/نص/ربع/ثلث]`','`سحب [مبلغ/كل/نص/ربع/ثلث]`','`تحويل @شخص مبلغ` — كل ساعة | حد 50%','`قرض مبلغ` — حد أقصى 20K','`سداد` (+5 سمعة)'].join('\n')},
          {name:'🏠 العقارات',value:['`بيوت` / `شركات` — السوق','`شراء بيت [h1-h20]` / `شراء شركة [c1-c20]`','`بيع [رقم]` — 70%','`ممتلكاتي`'].join('\n')},
          {name:'⛏️ المناجم',value:['`مناجم` — السوق (12 منجم)','`شراء منجم [m1-m12]` — واحد فقط!','`تعدين` — كل 3 ساعات','`مستودعي` / `مناجمي`','`بيع مورد [اسم] [كمية/كل]`','`بيع منجم` — 70%'].join('\n')},
    ...
    if (cmd==='ريست_الكل') {
      if(args[0]!=='تأكيد') return msg.reply('⚠️ هذا الأمر يرست **كل** الحسابات!\nللتأكيد اكتب: `ريست_الكل تأكيد`');
      db.prepare(`UPDATE users SET balance=5000, bank=0, loan=0, loan_due=0, reputation=100,
        last_salary=0, last_work=0, last_gamble=0, last_invest=0, last_trade=0,
        last_rob=0, last_collect=0, last_transfer=0, last_mine=0, protection_until=0`).run();
  ...
  setInterval(()=>{
    try{
      ...
      for(const u of overdue){
        ...
        if(prop){db.prepare('DELETE FROM properties WHERE id=?').run(prop.id);db.prepare('UPDATE government SET treasury=treasury+? WHERE id=1').run(prop.price);db.prepare('UPDATE users SET loan=MAX(0,loan-?) WHERE id=?').run(prop.price,u.id);}
        else{const cut=Math.floor((u.balance+u.bank)*0.3);db.prepare('UPDATE users SET balance=MAX(0,balance-?), loan=0, loan_due=0 WHERE id=?').run(cut,u.id);db.prepare('UPDATE government SET treasury=treasury+? WHERE id=1').run(cut);}
      }
